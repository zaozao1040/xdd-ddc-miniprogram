...
Page({
      ...
      requestModel.request(param, (resData) => {
        ...
        if (resData.mealType.orderStatus) {
          ...
          _this.setData({
            ...
            mealTypeItemShow:
              _this.data.mealTypeSmall[resData.mealType.mealType.toLowerCase()],
          });
  ...
  getSelectedFoods() {
    ...
    for (let y in tmpselectFoodsIndex.foodList) {
      // y 为选择的餐品index
    ...
    tmpselectFoodsIndex.payMoney = parseFloat(
      (
        this.data.allMenuData.totalMoney - this.data.allMenuData.deductionMoney
      ).toFixed(2)
    );
  ...
  handleCartAddfood(e) {
    ...
    if (canAddFlag) {
      ...
      tmpFoodTotalPrice = parseFloat(
        parseFloat(
          tmpFoodTotalPrice + parseFloat(tmp_oneFood.foodPrice)
        ).toFixed(2)
      );
  ...
  handleCalculateMoney_back(currnt_menuData) {
    ...
    if (
      ...
      //大于最低金额并且小于餐标
      if (
        currnt_menuData.totalMoney > currnt_menuData.mealType.lowestStandard &&
        currnt_menuData.totalMoney < currnt_menuData.mealType.standardPrice
      ) {
        // 退回的金额
        let oldTotalMoney_back = currnt_menuData.totalMoney_back;

        currnt_menuData.totalMoney_back = parseFloat(
          currnt_menuData.mealType.standardPrice - currnt_menuData.totalMoney
        );
        let totalMoney_back =
  ...
  goToMenuCommit() {
    if (this.data.totalCount > 0) {
      ...
      if (flag) {
        ...
        // })
        // 邱宁修改，增加一个字段 cantMealTotalMoney（不可使用餐标的总额） ，这里因为是补餐，而补餐是可以使用餐标的，所以设置为0
        wx.navigateTo({
          url:
            "/pages/menu/today/confirm/confirm?totalMoney=" +
            this.data.totalMoney +
            "&cantMealTotalMoney=0&totalMoneyRealDeduction=" +
            this.data.totalMoneyRealDeduction +
            "&realMoney=" +
