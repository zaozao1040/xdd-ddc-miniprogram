...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.getSetting({
      ...
      success(res) {
        let tmp = "SljAwyR2jA-_h4mNqJRa5tU096H-Ni90RoufB4VhOWY";
        if (res.subscriptionsSetting.itemSettings) {
          let result = res.subscriptionsSetting.itemSettings[tmp];
          if (result != "accept") {
            _this.setData({
              showDialog: true,
            });
    ...
    requestModel.request(params, (res) => {
      ...
      if (tmp_discountList.length > 0) {
        ...
        console.log(
          ...
          _this.data.page,
          _this.data.limit,
          page * limit >= res.amount,
          _this.data.hasMoreDataFlag
        );
      } else {
        _this.setData({
          discountList: [],
