...
Page({
  ...
  handleMinusfood(e) {
    ...
    let tmp_oneFood = this.data.allMenuData[tmp_mealTypeItem].foodList[
      menutypeIndex
    ].foodList[foodIndex];
    ...
    if (tmp_oneFood.foodCount > 0) {
      ...
      if (left) {
        //表示点击的是左侧标签
        this.data.allMenuData[tmp_mealTypeItem].foodList[
          leftmenuindex
      ...
      this.data.allMenuData[tmp_mealTypeItem].foodList[menutypeIndex].foodList[
        foodIndex
      ] = tmp_oneFood;
      // 总的数目减1
      ...
      if (tmp_oneFood.foodCount == 0) {
        ...
        tempselectFoodsIndex[tmp_mealTypeItem].foodList[
          menutypeIndex
        ] = tempselectFoodsIndex[tmp_mealTypeItem].foodList[
          menutypeIndex
        ].filter((item) => {
  ...
  handleAddfood(e) {
    ...
    let tmp_oneFood =
      _this.data.allMenuData[_this.data.mealTypeItem].foodList[menutypeIndex]
        .foodList[foodIndex];
    ...
    if (canAddFlag) {
      ...
      if (left) {
        //表示点击的是左侧标签
        _this.data.allMenuData[tmp_mealTypeItem].foodList[
          leftmenuindex
        ].foodList[leftfoodindex].foodCount += 1;
        _this.data.allMenuData[tmp_mealTypeItem].foodList[
          leftmenuindex
      ...
      // 先menu增1
      _this.data.allMenuData[tmp_mealTypeItem].foodList[menutypeIndex].foodList[
        foodIndex
      ] = tmp_oneFood;
      ...
      if (tmp_oneFood.foodCount == 1) {
        ...
        if (!_this.data.selectedFoodsIndex[tmp_mealTypeItem]) {
          ...
          tmp_copy.foodList = [];
          _this.data.selectedFoodsIndexCopy[tmp_mealTypeItem] = JSON.parse(
            JSON.stringify(tmp_copy)
          );
        }
        ...
        tmpselectFoodsIndex[tmp_mealTypeItem].foodList[menutypeIndex].push(
          foodIndex
        );
        _this.setData({
  ...
  getSelectedFoods() {
    ...
    for (let i in mealEnglistLabel) {
      ...
      if (tmpselectFoodsIndex[x]) {
        ...
        tmpselectFoodsIndex[x].deductionMoney = this.data.allMenuData[
          x
        ].deductionMoney;
        tmpselectFoodsIndex[x].payMoney = parseFloat(
          (
            this.data.allMenuData[x].totalMoney -
            this.data.allMenuData[x].deductionMoney
          ).toFixed(2)
        );
      }
    }
  ...
  handleCartMinusfood(e) {
    ...
    if (tmp_oneFood.foodCount > 0) {
      ...
      // 购物车中的减1
      let tmp_selectedFood = this.data.selectedFoodsIndex[tmp_mealTypeItem]
        .selectedFoods[tmp_selectedFoodIndex];
      // tmp_selectedFood.foodCount--; 【邱宁新增 修复today购物车加减后数据对不齐】
      ...
      this.data.selectedFoodsIndex[tmp_mealTypeItem].selectedFoods[
        tmp_selectedFoodIndex
      ] = tmp_selectedFood;
      this.data.selectedFoodsIndex[
        tmp_mealTypeItem
  ...
  verifyMealLabel() {
    ...
    for (let meal in this.data.allMenuData) {
      ...
      if (totalMoney > 0 && totalMoney < mealType.lowestStandard) {
        let t_title =
          this.data.appointment + " " + this.data.mealTypeSmall[meal];
        let t_content =
          "未达最低下单金额(¥" + mealType.lowestStandard + ")" + ",请继续选餐";
        this.setData({
          mealTypeItem: meal,
