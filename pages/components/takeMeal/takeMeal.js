...
Component({
  ...
  methods: {
    updateSelectedConfirmFlag() {
      ...
      if (
        this.data.takeMealLimitLists.BREAKFAST.length > 0 &&
        this.data.breakfastActiveQueue == -1
      ) {
        tmpFlag = false;
      } else if (
        this.data.takeMealLimitLists.LUNCH.length > 0 &&
        this.data.lunchActiveQueue == -1
      ) {
        tmpFlag = false;
      } else if (
        this.data.takeMealLimitLists.DINNER.length > 0 &&
        this.data.dinnerActiveQueue == -1
      ) {
        tmpFlag = false;
      } else if (
        this.data.takeMealLimitLists.NIGHT.length > 0 &&
        this.data.nightActiveQueue == -1
      ) {
        tmpFlag = false;
      }
      this.setData({
    ...
    handleConfirm() {
      ...
      if (this.data.breakfastActiveQueue != -1) {
        tmp_arr.push({
          ...
          des:
            "早:" +
            this.data.breakfastStartTime +
            "~" +
            this.data.breakfastEndTime,
        });
      }
      ...
      if (this.data.dinnerActiveQueue != -1) {
        tmp_arr.push({
          ...
          des:
            "晚:" + this.data.dinnerStartTime + "~" + this.data.dinnerEndTime,
        });
      }
